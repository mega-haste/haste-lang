...
class FunctionDef : public StatementNode
{
  public:
    ...
    FunctionDef(const Token &identifier, std::vector<TypedIdentifier> arguments, Type &&return_type,
                         Statement &&body)
    ...
    const std::string prettify(const int depth = 0) const override
    {
        ...
        return std::format("{}(func_dec name:{} args:({}) ret_type:{} body:({}))", ssss, identifier.value, args,
                           return_type->prettify(), body->prettify(depth + 4));
    }
};
...
class IfStatement : public StatementNode
{
  public:
    ...
    explicit IfStatement(Expression &&condition, Statement &&then, std::optional<Statement> &&otherwise)
    ...
    const std::string prettify(const int depth = 0) const override
    {
        return std::format("{}(if ({}) {} else {})", repeat_char(' ', depth), condition->prettify(),
                           then->prettify(depth), otherwise.has_value() ? otherwise.value()->prettify(depth) : "NONE");
    }
};
...
class ReturnStatement : public StatementNode
{
  public:
    ...
    const std::string prettify(const int depth = 0) const override
    {
        return std::format("{}(return {})", repeat_char(' ', depth), expr.has_value() ? expr.value()->prettify() : "NONE");
    }
};
