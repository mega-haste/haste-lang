...
Expression Parser::inline_if(void) {
    if (match({TokenType::If})) {
        ...
        consume(TokenType::Then, "Expected `then` after the condition of inline if.");
        auto consequent = expression();

        consume(TokenType::Else, "Expected `else` after the `then` cluase.");
        auto alternate = expression();

        return std::make_unique<InlineIf>(std::move(condition), std::move(consequent), std::move(alternate));
    }
...
Expression Parser::relational(void) {
    ...
    while (match({TokenType::LessThan, TokenType::LessThanOrEq, TokenType::GreaterThan, TokenType::GreaterThanOrEq})) {
        const Token &op = previous();
...
Expression Parser::call(void) {
    ...
    while (true) {
        if (match({TokenType::OpenParen})) {
            ...
            const Token &close_bracket = consume(TokenType::CloseSquareBracket, "Expected closing bracket ']'.");
            expr = std::make_unique<SubscriptingExpression>(std::move(expr), open_bracket, close_bracket,
                                                            std::move(index));
        } else if (match({TokenType::Dot})) {
            const Token &name = consume_identifier("Expected property name after the dot '.'.");
            expr              = std::make_unique<MemberAccessExpression>(std::move(expr), previous(), name);
        } else {
            break;
        }
    }
...
Expression Parser::scope_resolution(void) {
    ...
    while (match({ TokenType::ColonColon })) {
        ...
        left = std::make_unique<ScopeResolutionExpression>(
            std::move(left),
            colon_colon,
            ident
        );
...
Expression Parser::primary(void) {
    ...
    if (match({TokenType::CharLit, TokenType::StringLit, TokenType::IntLit, TokenType::FloatLit}))
...
Expression Parser::parse_tuple(std::vector<Expression> &&exprs) {
    ...
    const Token &closing_parentheses = consume(TokenType::CloseParen, "Expected ')' after expression.");
    return std::make_unique<TupleExpression>(std::move(exprs), closing_parentheses);
}
